...
mu_assert(array->max==2, "Max value isnt teh one as expected");
...
mu_assert(val1!=NULL, "failed to make a new element in darray");
...
*val=i*333; // to the value pointed by thsi one add the value i*	
...
int val=0; 
val=fun();
mu_assert(val==5, "Shoudnt get NULL");
